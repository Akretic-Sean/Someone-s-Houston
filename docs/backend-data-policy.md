...
- The discovered HPD summary ends in 2024, so it fails the proposed 18-month limit on 2026-09-19. Retain it as a discovery reference. Newer 2025/2026 CSV headers and small samples have since been verified; full-period coverage, coordinates, and aggregation still need validation before scoring. See the [source shortlist](source-shortlist.md).
...
- The reference/context schema, bounded staging retention and current-feed scheduler are implemented. Future scoring-metric/report tables, report expiration and their version-retention jobs remain unimplemented. See `matrix-readiness.md` for the current scoring-data gaps.
