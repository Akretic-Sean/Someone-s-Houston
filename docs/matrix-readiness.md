...
The user's 2026-09-19 screenshot confirms the existing frontend category IDs and supersedes the earlier 25/20/10/15/15/7/5/3 draft. Current configuration: [report-priorities.v1.json](../backend/data/reference/report-priorities.v1.json), mirrored in Supabase `report_category_config`. Raw slider defaults total 54; normalize across all eight weights and round only for display.
...
These are usable facts, not eight complete scores. Every category returns missing inputs, limitations, sources, dates and a null score. Report generation/scoring remains separate. Never renormalize around a different set of missing criteria for each neighborhood. Any partial comparison must use the same supported metrics across all candidates and disclose omissions.
