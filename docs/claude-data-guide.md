...
4. Start Claude Code from the repository root, use `/context` to confirm the root instructions loaded, and `/mcp` to inspect/connect `hou-match-neighborhoods`. Reconnect after rebuilding the server. All six product tools should be visible, including `get_neighborhood_evidence`.
...
| Report status | Says taxes, routing and stored reports are unavailable; the six MCP tools supply facts, while the separately implemented shared model produces provisional neighborhood scores |
