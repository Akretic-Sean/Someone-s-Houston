...
4. Start Claude Code from the repository root, use `/context` to confirm the root instructions loaded, and `/mcp` to inspect/connect `hou-match-neighborhoods`. Reconnect after rebuilding the server. All four product tools should be visible.
...
> Read CLAUDE.md and docs/api.md. Using hou-match-neighborhoods, resolve Midtown's official ID, get its economic profile, list its parks with source dates, and check current conditions. Also check Hidden Valley's rent. Explain which report fields remain unsupported. Use actual tool results, cite the observation period separately from check time, and keep missing/stale values unavailable. Do not use mock report values as fallbacks.
...
| Report status | Says taxes, routing, stored reports and recommendation scores are not produced by the current four tools |
