...
## All-88 shared calculation (implemented, opt-in)

`scoreAllNeighborhoods(payload, options, now?)` in `shared/scoring-coverage.mjs` consumes the unchanged scoring RPC response and existing `ScoringOptions`. Returns `policyVersion: common-coverage-v1`, `label`, `notice`, `evaluatedAt`, `requestedWeights`, `requestedEffectiveWeights`, `retainedWeightFraction`, `includedCategories`, `omittedCategories` (ID, label, requested weight and affected IDs/names/reasons), `requested` (strict ScoringResult), and `comparison` (88-row ScoringResult using globally common categories). No new RPC, data fill, credential, database write or MCP mode. Throws existing validation/expiry errors and `NO_COMMON_CATEGORIES` when no selected positive-weight category is fully covered. See [the complete frontend walkthrough](all-88-frontend-guide.md).
