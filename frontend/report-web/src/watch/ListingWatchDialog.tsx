...
export default function ListingWatchDialog({
  ...
  const ready = hasListingWatch && email.trim().length > 0 && consent && selected.length > 0;
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      onDone('The listing watch service accepted your request.');
      onClose();
    } catch (err) {
      setError(
        err instanceof WatchError
          ? err.kind === 'unconfigured'
            ...
            : err.message
          : 'Could not confirm whether the service accepted the request. Please try again later.',
      );
    } finally {
      setBusy(false);
    }
    ...
    <dialog ref={ref} className="watch-dialog" onCancel={onClose} onClose={onClose}>
      <form onSubmit={submit} noValidate>
        ...
        <p className="section-lede">
          {hasListingWatch
            ? 'Request a weekly listing digest for the ranked neighborhoods you select. Listing checks and delivery are handled by the separately connected watch service.'
            : 'Listing Watch is not connected in this build. You can preview the preferences below, but no watch is created and no email is sent.'}
        </p>
        ...
        <label className="consent">
          ...
          {WATCH_EXPIRY_WEEKS} weeks. This opt-in request is sent only when the watch service is connected.
        </label>

        {!hasListingWatch ? (
          <p className="source" style={{ marginBottom: 10 }}>
            Unavailable in this build. Connect the watch service before creating a weekly watch.
          </p>
        ) : null}
